...
つまり、データの長さ(バイト単位)が正確に2の倍数ではない場合、データの最後にデータの長さが正確に2の倍数となるように`0`bをパディングする。
...
この場合、[4.1.1.](#sec-4-1-1)でデータを記述するかのように実際のデータの長さを整数型([3.1.参照](#sec-3-1))によって記述する。
...
名前の長さは65535バイトが上限であるため、最初のワードは常に `4E 02`h となる。
