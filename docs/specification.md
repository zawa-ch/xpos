...
-	[5 XPOS データフォーマット](#sec-5)
	-	[5.1 ワード](#sec-5-1)
	-	[5.2 識別子](#sec-5-2)
	-	[5.3 整数型 オブジェクト記述](#sec-5-3)
		-	[5.3.1 単一ワード整数型](#sec-5-3-1)
		-	[5.3.2 多倍ワード整数型](#sec-5-3-2)
	-	[5.4 バイナリ型 オブジェクト記述](#sec-5-4)
		-	[5.4.1 単一ワード長バイナリ型](#sec-5-4-1)
		-	[5.4.2 多倍ワード長バイナリ型](#sec-5-4-2)
	-	[5.5 文字列型 オブジェクト記述](#sec-5-5)
		-	[5.5.1 単一ワード長文字列型](#sec-5-5-1)
		-	[5.5.2 多倍ワード長文字列型](#sec-5-5-2)
	-	[5.6 実数型 オブジェクト記述](#sec-5-6)
	-	[5.7 コレクション型 オブジェクト記述](#sec-5-7)
		-	[5.7.1 単一ワード長コレクション型](#sec-5-7-1)
		-	[5.7.2 多倍ワード長コレクション型](#sec-5-7-2)
	-	[5.8 名前型 オブジェクト記述](#sec-5-8)
	-	[5.9 拡張オブジェクト型 オブジェクト記述](#sec-5-9)
	-	[5.10 マジックナンバー](#sec-5-10)
	-	[5.11 XPOS データ記述](#sec-5-11)
-	[6 仕様の策定者への勧告](#sec-6)
	...
	-	[6.5 実数型の扱い](#sec-6-5)
-	[付録A. 拡張オブジェクト型使用例: CRCチェックサム](#appendix-a)
...
### <a id="sec-5-3"></a> 5.3. Integer Type Object Notation - 整数型 オブジェクト記述

整数型のデータの記述方法は2種類ある。
一方は単一のワードに値を記述する方法、もう一方は以降のワードに値を記述する方法である。

#### <a id="sec-5-3-1"></a> 5.3.1. Short Integer Type - 単一ワード整数型
...
#### <a id="sec-5-3-2"></a> 5.3.2. Long Integer Type - 多倍ワード整数型

複数のワードに値を記述する場合、識別子は`49`h (ASCII: "I")である。
続く3バイトには、記述する値のデータの長さをバイト単位で記述し、次のワード以降に値を記述する。

データの長さは単一ワード整数型([5.3.1.参照](#sec-5-3-1))で値を格納するときのように格納される。
...
### <a id="sec-5-4"></a> 5.4. Binary Type Object Notation - バイナリ型 オブジェクト記述

バイナリ型のデータの記述方法は2種類ある。
これはデータの長さをどちらの整数型の記述([5.3.参照](#sec-5-3))で表すかによるものである。

#### <a id="sec-5-4-1"></a> 5.4.1. Short Binary Type - 単一ワード長バイナリ型
...
#### <a id="sec-5-4-2"></a> 5.4.2. Long Binary Type - 多倍ワード長バイナリ型

バイナリの長さが8388608バイト以上である場合、識別子として`42`h (ASCII: "B")を用いる。
以降はデータの長さをバイト単位で記述する。
データの長さを表す整数型の最後のワードに続いてデータを記述する。

データの長さは多倍ワード整数型([5.3.2.参照](#sec-5-3-2))で値を格納するときのように格納される。
...
### <a id="sec-5-5"></a> 5.5. String Type Object Notation - 文字列型 オブジェクト記述

文字列型のデータの記述方法は2種類ある。
これは文字列のデータの長さをどちらの整数型の記述([5.4.参照](#sec-5-4))で表すかによるものである。

#### <a id="sec-5-5-1"></a> 5.5.1. Short String Type - 単一ワード長文字列型

文字列の長さが8388608バイト未満である場合、識別子として`73`h (ASCII: "s")を用いる。
続く3バイトには、記述する値のデータの長さをバイト単位で記述し、次のワード以降に値を記述する。

データの長さは単一ワード整数型([5.3.1.参照](#sec-5-4-1))で値を格納するときのように格納される。
...
### <a id="sec-5-6"></a> 5.6. Real Number Type Object Notation - 実数型 オブジェクト記述
...
### <a id="sec-5-7"></a> 5.7. Collection Type Object Notation - コレクション型 オブジェクト記述

コレクション型のデータの記述方法は2種類ある。
これはコレクション型のデータの長さをどちらの整数型の記述([5.4.参照](#sec-5-4))で表すかによるものである。

#### <a id="sec-5-7-1"></a> 5.7.1. Short Collection Type - 単一ワード長コレクション型
...
#### <a id="sec-5-7-2"></a> 5.7.2. Long Collection Type - 多倍ワード長コレクション型
...
### <a id="sec-5-8"></a> 5.8. Name Type Object Notation - 名前型 オブジェクト記述

コレクション型のオブジェクト内で識別子が`4E`h (ASCII: "N")となっている文字列型オブジェクトが存在する場合、直後のオブジェクトに付けられる名前として扱う。

データの内容は単一ワード長文字列型の記述([5.5.1.参照](#sec-5-5-1))のように記述する。
...
### <a id="sec-5-9"></a> 5.9. Extended Object Type Object Notation - 拡張オブジェクト型 オブジェクト記述
...
データの長さは単一ワード整数型([5.4.1.参照](#sec-5-4-1))で値を格納するときのように格納される。
...
### <a id="sec-5-10"></a> 5.10. Magic Number - マジックナンバー
...
### <a id="sec-5-11"></a> 5.11. XPOS Data Notation - XPOS データ記述
...
プリミティブ型オブジェクトは必須であり、0個以下であっても2個以上であってもならないが、記述するプリミティブ型オブジェクトはいずれでもよい。
...
ルートとなるプリミティブ型オブジェクトに名前をつけてもよい。
この場合、マジックナンバーの直後に名前が入り、その後にルートとなるプリミティブ型オブジェクトが記述される。

ルートとなるプリミティブ型オブジェクトの最後のワード以降のデータはすべて無視するものとする。
つまり、ルートとなるプリミティブ型オブジェクトに拡張型オブジェクトを付加することはできない。
...
拡張オブジェクト型はデータの再現に必ずしも必要ではないデータを、プリミティブ型オブジェクトに付加する形式で記述するデータである。
例えば、三次元空間の座標を表すオブジェクトをXPOSで表現する場合、各座標軸の値をプリミティブ型オブジェクトで表現しなければならないが、それらから算出できるゼロ点からの距離や偏角は拡張オブジェクト型に格納してよいということである。
これらの拡張オブジェクト型をどのように扱う(扱わせる)かは、策定者の自由とする。
つまり、拡張オブジェクト型に格納できるデータを必ず格納するように仕様を策定することも、特定の拡張オブジェクト型の読み込みを実装者に必ず行わせるよう使用を策定することも、策定者の自由である。
...
## <a id="appendix-a"></a> 付録A. Example of Extended Object Type: CRC Checksum - 拡張オブジェクト型使用例: CRCチェックサム

ここでは拡張オブジェクト型の使用例として、プリミティブ型オブジェクトの検証を行うCRCチェックサムのオブジェクトを定義する。
...
名前や他の拡張オブジェクト型のオブジェクトはチェックサムの計算に含めない。
...
この拡張オブジェクト型の内容は次のようになる。

-	拡張型オブジェクトのヘッダ(`78 00 00 08`h)
-	拡張識別子(`43 33 32 63`h)
